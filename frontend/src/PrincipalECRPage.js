...
const fetchFac=async(dId)=>{
  await axios.get(`http://10.167.1.2:1234/seminar/findFacWithDept/${dId}`)
          .then((response) => {
  ...
  const fetchDept=async()=>{
      const t = await axios.get("http://10.167.1.2:1234/ecrFilter/getDeptList")
...
const [totalPages, setTotalPages] = useState(1);
      ...
      const fetchData = async (page) => {
        try {
          ...
          const response = await axios.get(`http://10.167.1.2:1234/seminar/principalecr/${empId}?page=${page}`);
